...
-   RWMutex solve it, but they [do not scale](https://www.1024cores.net/home/lock-free-algorithms/reader-writer-problem). In order to scale RWMutex,
...
#### Optimistic locking mechanism (Allow conflicts, better scalability) {#optimistic-locking-mechanism--allow-conflicts-better-scalability}
...
-   MMVC
    -   Allows several versions of an object to exist at the same time.
    -   Readers acquire the current version and work with it as much as they want
    -   Writer can create and publish a new version of an object
