...
-   `inodes` are stored on disk, contain the actual location(not the path!) of the file. Each FS has its own way to store it.
    ...
    -   FS such as ZFS, btrfs, etc omit a fixed-size inode table. They use `B trees` / `B+ trees`
    -   `inodes` are stored per FS, thus are FS dependant. Means the same `inode no`  can be found on multiple FS().
-   `inode` structure
